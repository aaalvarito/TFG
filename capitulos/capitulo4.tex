...
\begin{figure} [h!]
  \begin{center}
    \includegraphics[width=14cm]{figs/com_plc_ur}
  \end{center}
  \caption{\centering Configuración de los módulos de comunicación del UR en Tia Portal.}
...
\begin{figure} [h!]
  \begin{center}
    \includegraphics[width=13cm]{figs/ur_conectado_plc}
  \end{center}
  \caption{\centering Conexión establecida entre el UR y el PLC. \cite{guia_profinet}}
  \label{fig:ur_conectado_plc}
\end{figure} 

Seguidamente se muestra una tabla que resume todos los mensajes intercambiados entre los dos PLCs dentro del ciclo global del sistema: \\
...
\begin{tabular}{|M{1.5cm}|M{1.75cm}|M{3.75cm}|M{6.5cm}|}
\hline
\textbf{Emisor} & 
\textbf{Receptor} & 
\textbf{Mensaje} & 
...
\caption{Intercambio de mensajes entre el PLC 1 y el PLC 2.}
\label{cuadro:mensajes_plcs}
\end{center}
\end{table}

Cada fila representa una comunicación concreta entre el PLC 1 (estación de distribución) y el PLC 2 (estación de unión). Los mensajes son de tipo booleano o entero y permiten coordinar el paso de piezas entre estaciones. Send\_fase\_1 se utiliza para indicar y confirmar el traslado de una pieza aceptada. Tipo\_de\_pieza transmite el tipo de pieza (negra, rosa o metálica) mediante un valor entero. Otros mensajes, como Secuencia\_acabada o Pieza\_mal\_orientada, informan sobre el estado del proceso o si la pieza debe descartarse. Cada mensaje importante cuenta con su correspondiente confirmación (ACK) para asegurar la correcta recepción y sincronización entre ambos PLCs.

\newpage

También se ha realizado una tabla que resume todos los mensajes intercambiados entre los dos el PLC 1 (estación distribución) y el cobot UR5e dentro del ciclo global del sistema: 
...
\caption{Intercambio de mensajes entre el PLC 1 y el UR5e.}
\label{cuadro:mensajes_plc_ur}
\end{center}
\end{table}
...
\begin{figure} [h!]
  \begin{center}
    \includegraphics[width=13cm]{figs/grafcet_distribucion}
  \end{center}
  \caption{\centering Grafcet del funcionamiento de la estación distribución.}
  \label{fig:grafcet_distribucion}
\end{figure} 

En el anexo \ref{sec:anexo_distribucion} se detallan las ecuaciones lógicas del Grafcet de la estación distribución. Además, en el material suplementario de la memoria está el diagrama de contactos que implementa el Grafcet en TIA Portal llamado \textbf{codigo\_estacion\_distribucion}.
...
\clearpage
 ...
 \begin{figure}[h!]
  \includegraphics[width=14.5cm]{figs/grafcet_union}
  \caption{\centering Grafcet de funcionamiento de la estación unión.}
  \label{fig:grafcet_union}
\end{figure}

En el anexo \ref{sec:anexo_union} se detallan las ecuaciones lógicas del Grafcet de la estación distribución. Además, en el material suplementario de la memoria está el diagrama de contactos que implementa el Grafcet en TIA Portal llamado \textbf{codigo\_estacion\_union}.
...
Para activar la parada de emergencia del sistema, se ha definido una condición en cada etapa del Grafcet, si se pulsa el botón de "parada de emergencia", todas las etapas se desactivan inmediatamente. Esto provoca que se anulen también las señales enviadas a los actuadores, deteniendo por completo el funcionamiento del sistema.

\clearpage

\subsubsection{Aplicación de la Guía GEMMA}

Una vez descritos en detalle todos los posibles estados que puede adoptar el sistema a lo largo de su funcionamiento, se presenta a continuación la imagen \ref{fig:guia_gemma_final} la cual muestra gráficamente todas las transiciones entre dichos estados. Esta representación visual resulta especialmente útil para comprender de forma global el comportamiento del sistema automatizado en sus distintas fases operativas. 

\begin{figure}[h!]
  \begin{center}
  	\includegraphics[width=14.7cm]{figs/guia_gemma_final}
  \end{center}
  \caption{\centering Esquema de transiciones de la aplicación de la Guía GEMMA en el sistema.}
  \label{fig:guia_gemma_final}
\end{figure}

En la figura \ref{fig:guia_gemma_final}, los elementos de color verde corresponden al proceso de funcionamiento normal, los de color amarillo al proceso de parada o de puesta en marcha, y los de color rojo al proceso de detección de defectos o fallos. La imagen también cuenta con una leyenda explicando las acciones para transicionar entre estados.
...
Las coordenadas de la posición de las piezas en el paletizado se han tomado de forma relativa a la coordenada del primer brick de leche. La primera coordenada se definió en la posición que se quería colocar el primer elemento del palé, y a partir de esta y tomándola de referencia, se han definido las 5 coordenadas restantes. Al utilizar el sistema de coordenadas del primer punto se han podido colocar elementos paralelos y perpendiculares al primero, ayudando a aportar gran estabilidad y orden al palé. Esta técnica también ha ayudado mucho a establecer la misma altura de todos los bricks de leche y asegurarse así que se coloquen de forma uniforme. Adicionalmente, se han establecido algunos puntos de paso también utilizando este sistema de coordenadas para establecer movimientos rectilíneos y dar estabilidad a las piezas durante el movimiento. Seguidamente se muestra una tabla que refleja las coordenadas de las posiciones de cada pieza que forman el palé, tomando como referencia la primera posición para definir el resto de ellas:
